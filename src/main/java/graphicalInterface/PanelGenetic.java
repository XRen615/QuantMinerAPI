...
public class PanelGenetic extends DatabasePanelAssistant { // step 4
	...
	public RuleTester m_calculateur = null; // A very important member, it
												// derives from thread
	private int m_iMaxReglesTestees = 0; // max number of testing rule?
	private int m_iIndiceRegleAffichee = 0; // Affichee means display--the index
											// of the rule being displayed
	private boolean m_bResultatAffiche = false; // Indicate que la total des
												// r�gles calcul�es ont �t�
												// affich�es
	...
	class IndicateurCalculReglesGenerique extends RuleTester.IndicateurCalculRegles {
		...
		public void IndiquerFinCalcul() {
			...
			fenetreFinCalcul = new DialogEndComputeRules(m_panneauParent.m_contexteResolution.m_fenetreProprietaire,
					true);

			if ((m_panneauParent.m_contexteResolution.m_fenetreProprietaire.getExtendedState()
					& java.awt.Frame.ICONIFIED) != 0) {
				m_panneauParent.m_contexteResolution.m_fenetreProprietaire
		...
		public void EnvoyerInfo(String sNouvelleInfo) {
			...
			// X.R Print out parameter info
			System.out.println(sNouvelleInfo);
		}
	...
	public PanelGenetic(ResolutionContext contexteResolution) {
		...
		switch (contexteResolution.m_iTechniqueResolution) { // which algorithm
																// to use
		...
		case ResolutionContext.TECHNIQUE_RECUIT_SIMULE:
			super.DefinirEtape(4, "Mining rules using a simulated annealing algorithm",
					ENV.REPERTOIRE_AIDE + "computation.htm");
			break;
		}

		super.DefinirPanneauPrecedent(MainWindow.PANNEAU_CONFIG_TECHNIQUE); // previous
																			// step
																			// is
																			// step
																			// 3
		super.DefinirPanneauSuivant(MainWindow.PANNEAU_RESULTATS); // next step
																	// is step 5
		...
		ActionListener tacheProgrammee = new ActionListener() { // tache means
																// task
		...
		// listeners.
		new javax.swing.Timer(1000, tacheProgrammee).start(); // call
																// tacheProgrammee
																// to do refresh
																// every second
	}
	...
	private void initComponents() {// GEN-BEGIN:initComponents
		...
		jZoneTexteRegles = new javax.swing.JTextArea(); // text area to display
														// rules
		jScrollPaneContexte = new javax.swing.JScrollPane(); // context display
																// panel
		jTextAreaContexte = new javax.swing.JTextArea(); // text area to display
															// context
		jProgressResolution = new javax.swing.JProgressBar(); // progress bar
		jButtonTravaillerFond = new javax.swing.JButton(); // work as a
															// background task
	...
	// start calculation
	// X.R I changed a button to startCalculation()
	public void startCalculation() {// GEN-FIRST:event_jBoutonDemarrerActionPerformed
			...
			if (!m_bResultatAffiche) {
				...
				if ((m_calculateur.m_bEnExecution) || (bResultatDisponible)) {
					...
					if (m_iMaxReglesTestees == 0) {
						...
						sIndicateur = String.valueOf(m_calculateur.m_iNombreReglesTestees) + " tested rules /  "
					...
					do {
						...
						if (regle != null) {
							...
							AjouterRegle("\n");
							// X.R Print out results
							String result = "***********************Rules Mined**********************"+"\n";
							result+= String.valueOf(m_iIndiceRegleAffichee + 1) + ".  " + regle.toString() + "\n";
	...
	void ArrangerDisposition() {
		int iDeltaPosX = 0; // Diff�rence de positionnement horizontal entre la
							// position id�ale et celle de l'�diteur de
							// formulaires
		int iDeltaPosY = 0; // Diff�rence de positionnement vertical entre la
							// position id�ale et celle de l'�diteur de
							// formulaires
		...
		jScrollPaneContexte.setBounds(jScrollPaneContexte.getX() - iDeltaPosX, jScrollPaneContexte.getY() - iDeltaPosY,
				super.m_zoneControles.width, jScrollPaneContexte.getHeight());
		...
		jButtonTravaillerFond.setLocation(
				(super.m_zoneControles.width + super.m_zoneControles.x) - jButtonTravaillerFond.getWidth(),
				jButtonTravaillerFond.getY() - iDeltaPosY);

		jProgressResolution.setBounds(jProgressResolution.getX() - iDeltaPosX, jProgressResolution.getY() - iDeltaPosY,
				super.m_zoneControles.width, jProgressResolution.getHeight());

		jScrollPaneRegles.setBounds(jScrollPaneRegles.getX() - iDeltaPosX, jScrollPaneRegles.getY() - iDeltaPosY,
	...
	// Outrepassement de la m�thode m�re pour l'annulation du processus � partir
	// de ce panneau :
	...
	//getter of rules X.REN
	public ArrayList getRules(){
		return m_calculateur.getM_listeRegles();
		}
